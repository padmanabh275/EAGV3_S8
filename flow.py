...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        for child_skill in src_def.internal_successors:
            ...
            added.append(nid)
            # Formatter plans often reference the coder node; after the
            # sandbox runs, the final answer must come from stdout, not
            # from {"code": "..."}. Rewire pending formatters that still
            # depend on this coder so they wait on the sandbox instead.
            if child_skill == "sandbox_executor":
                for fmt_nid, fmt_d in self.g.nodes(data=True):
                    if fmt_d.get("skill") != "formatter" or fmt_d["status"] != "pending":
                        continue
                    inputs = list(fmt_d.get("inputs") or [])
                    if src_nid not in inputs:
                        continue
                    fmt_d["inputs"] = [nid if x == src_nid else x for x in inputs]
                    if self.g.has_edge(src_nid, fmt_nid):
                        self.g.remove_edge(src_nid, fmt_nid)
                    self.g.add_edge(nid, fmt_nid)
